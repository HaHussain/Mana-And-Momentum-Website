To make a character within CREST, you can start from couple different starting points

# Creating a character's personality

# Creating a character's mechanics

First pick two different [[Classes/index|Classes]].

Roll ((3d4 drop lowest ) - 5) 6 times, recording them, then assigning one to each of the 6 [[Rules/Characters/index#Stats|Stats]].
Or take the standard array of \[+3,+2,+1,+1,0,-1]
Pick a [[Ancestry]] and [[Culture]] and update Stats accordingly.

Your [[Health Points|Max HP]] is equal to 10 + [[Fortitude]] + Class 1's Health Bonus + Class 2's Health Bonus

Your [[Seek]] is 1 + [[Awareness]] squares (minimum 1), if your [[Awareness]] is less than 0, then your [[Seek]] becomes [[Orthogonal]].
Your [[Item Points|Max Item Points]] is equal to 2 + [[Intelligence]] (min 0).
Your [[Weight|Max Weight]] is equal to 10 + 2 * [[Strength]].
Your [[Exert#Exertion|Maximum Exertion]] is equal to 2 + [[Fortitude]] (min 1).
Your [[Resolve]] is equal to 1 + [[Level Bonus|LB]] + [[Instinct]] / 2 (min 1).
Your [[Level Bonus]] is +2.

Grab [[Items/index|Items]] up to your [[Weight|Max Weight]] 

Calculate your [[Defence]], [[Deflection]] and [[Sturdy]] based on your [[Armour]].

Apply 4 [[Levelling Up#Skill Points|Skill Points]] to [[Rules/Characters/Skills/index|Skills]] of your choice with a maximum of 2 points to a single skill.

Record your [[Class Keywords|Class Keyword]] for each class.
Choose one of the three [[Class Trigger|Class Triggers]] for each class.
Gain one of the two [[Fortress]] bonus by deciding between your two classes.

Finally record the abilities gained by each class including any [[Arts/Paradigms/index|Paradigms]] and [[Arts/Spells/index|Spells]] gained.

Assign at least one [[Bond]] to a [[Rules/Characters/index|Character]] or a [[Factions|Faction]].